...
<p>$$\begin{aligned}
& \lambda_{coord} \sum_{i=0}^{S^2} \sum_{j=0}^{B} \Bbb{1}_{ij}^{obj} \Big[(x_i - \hat{x}_i)^2 + (y_i - \hat{y}_i)^2\Big] \\
& + \lambda_{coord} \sum_{i=0}^{S^2} \sum_{j=0}^{B} \Bbb{1}_{ij}^{obj} \Big[(\sqrt{w_i} - \sqrt{\hat{w}_i})^2 + (\sqrt{h_i} - \sqrt{\hat{h}_i})^2\Big] \\
& + \sum_{i=0}^{S^2} \sum_{j=0}^{B} \Bbb{1}_{ij}^{noobj} (C_i - \hat{C}_i)^2 \\
& + \lambda_{noobj} \sum_{i=0}^{S^2} \sum_{j=0}^{B} \Bbb{1}_{ij}^{noobj} (C_i - \hat{C}_i )^2 \\
